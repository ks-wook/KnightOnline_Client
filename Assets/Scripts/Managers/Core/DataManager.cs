...
{
    public bool isDataSheetInitialized = false;
    ...
    public void Init()
    {
        ...
        // 스킬 데이터 처리
        SkillDict = LoadJson<Data.SkillData, int, Data.Skill>("SkillData.encrypted").MakeDict();
    ...
    Loader LoadJson<Loader, Key, Value>(string path) where Loader : ILoader<Key, Value>
    {
        ...
        Debug.Log("복호화 완료 : " + decryptedJsonFile);

        if (path == "SkillData.encrypted")
            Debug.Log("스킬 데이터 " + decryptedJsonFile);
